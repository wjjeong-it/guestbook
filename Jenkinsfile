...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="wjjeong/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/wjjeong-it/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/wjjeong-it/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://wjjeong-it:${GITHUB_ACCESS_TOKEN}@github.com/wjjeong-it/guestbook-config.git"
                    '''
                }
            }
        }
